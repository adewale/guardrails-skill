...
```sh
python3 skill-eval-harness/skill_benchmark.py validate guardrails-skill/evals/shared-benchmark.json
```

Prepare paired run tasks:

```sh
python3 skill-eval-harness/skill_benchmark.py prepare guardrails-skill/evals/shared-benchmark.json --split tune --out /tmp/guardrails-skill-tasks.jsonl
```

Include ablation variants when running a focused regression check:

```sh
python3 skill-eval-harness/skill_benchmark.py prepare guardrails-skill/evals/shared-benchmark.json --split tune --include-ablations --out /tmp/guardrails-skill-ablation-tasks.jsonl
```
...
```sh
python3 skill-eval-harness/skill_benchmark.py benchmark guardrails-skill/evals/shared-benchmark.json --runs guardrails-skill/eval-runs/latest --out /tmp/guardrails-skill-benchmark.json
```
