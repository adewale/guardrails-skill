...
hooks:
  ...
  PreToolUse:
    - matcher: "Bash"
      hooks:
        - type: prompt
          prompt: "If git commit: verify full suite passed, secrets scanned, commit message conventional, new code reachable. If bug fix (fix: prefix or fixing a reported bug): verify retrospective — detection gap identified, concrete prevention added, pattern scan performed. Else allow. $ARGUMENTS"
